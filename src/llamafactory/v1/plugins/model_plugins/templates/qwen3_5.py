...
def _render_message_content(
    ...
    *,
    ...
    if any(content["type"] == "audio_url" for content in message["content"]):
...
class Qwen35TemplateRenderer(BaseTemplateRenderer):
    """Qwen3.5 renderer organized around the native chat-template sections."""
    ...
    def render_system_and_tools(
        ...
        Qwen35TemplateRenderer.append_segment(processor, model_input, temp_str, temp_weight)
    ...
    def render_conversation(processor: Processor, model_input: ModelInput, messages: list[Message]) -> None:
        ...
        for turn_idx, message in enumerate(messages):
            ...
            if message["role"] == "user":
                Qwen35TemplateRenderer._render_user(processor, model_input, message)
            elif message["role"] == "assistant":
                Qwen35TemplateRenderer._render_assistant(processor, model_input, message, turn_idx > last_query_index)
            elif message["role"] == "tool":
                Qwen35TemplateRenderer._render_tool(processor, model_input, messages, message, turn_idx)
            else:
    ...
    def render_generation_prompt(
        ...
        Qwen35TemplateRenderer.append_segment(processor, model_input, temp_str, 0.0)

    @staticmethod
    def _render_user(processor: Processor, model_input: ModelInput, message: Message) -> None:
        ...
        if len(message_images) != 0:
            model_input.setdefault("images", []).extend(message_images)
        if len(message_videos) != 0:
            model_input.setdefault("videos", []).extend(message_videos)
        Qwen35TemplateRenderer.append_segment(
            processor,
            model_input,
            "<|im_start|>user\n" + message_text + "<|im_end|>\n",
            message.get("loss_weight", 0.0),
        )
    ...
    def _render_assistant(
        ...
        temp_str += "<|im_end|>\n"
        Qwen35TemplateRenderer.append_segment(processor, model_input, temp_str, message.get("loss_weight", 1.0))
    ...
    def _render_tool(
        ...
        Qwen35TemplateRenderer.append_segment(processor, model_input, temp_str, message.get("loss_weight", 0.0))
...
def render_qwen3_5_messages(
    ...
    return Qwen35TemplateRenderer.render_messages(processor, messages, tools, is_generate, enable_thinking)
...
def parse_qwen3_5_message(generated_text: str) -> Message:
    ...
    return Qwen35TemplateRenderer.parse_message(generated_text)
