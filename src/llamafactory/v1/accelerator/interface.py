...
from torch.distributed import barrier, destroy_process_group, get_rank as dist_get_rank, get_world_size as dist_get_world_size, init_process_group, new_group
...
class DistributedInterface:
    ...
    def _init_extra_groups(self) -> None:
        ...
        # 通过全局 init_device_mesh 构建正交的 edp, efsdp, ep, cp 网格
        # 避免嵌套 new_group 在 NPU HCCL 引起 communicator 初始化顺序错乱。
    ...
    def get_expert_meshes(self) -> tuple[DeviceMesh | None, DeviceMesh | None]:
        """Return cached expert-side EP/EFSDP meshes for MindSpeed."""
